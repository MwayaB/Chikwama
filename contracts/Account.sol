...
contract Account{
  ...
  /*  function createChikwama(address _add, uint256 _value, uint256 _id, uint8 _chikwamaType, uint256 _pin) public returns(bool)
    ...
    function createChikwama( uint256 _value, uint256 _id, uint8 _chikwamaType, uint256 _pin) public returns(bool)
     {
         if (accounts[keccak256(_id)]==0x0)
         {
             accounts[keccak256(_id)] = msg.sender;
             chikwama[msg.sender][keccak256(_id)].add = msg.sender;
             chikwama[msg.sender][keccak256(_id)].id = keccak256(_id);
             chikwama[msg.sender][keccak256(_id)].value = _value;
             chikwama[msg.sender][keccak256(_id)].chikwamaType = _chikwamaType;
             chikwama[msg.sender][keccak256(_id)].pin = keccak256(_pin);
             accountCount++;
             CreateAccount(keccak256(_id),msg.sender);
             return true;
         }

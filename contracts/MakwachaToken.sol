...
contract Chikwama{
    ...
    mapping(address => mapping(uint256 => Account)) chikwama;
    
    event CreateAccount(uint256 NationalID, address ChikwamaAddress);
    ...
    function createChikwama(address _add, uint256 _value, uint256 _id, uint8 _chikwamaType, uint256 _pin) public returns(bool)
    {
        ...
        {
            accounts[_id] = _add;
            chikwama[_add][_id].add = _add;
            chikwama[_add][_id].id = _id;
            chikwama[_add][_id].value = _value;
            chikwama[_add][_id].chikwamaType = _chikwamaType;
            chikwama[_add][_id].pin = _pin;
            accountCount++;
            CreateAccount(_id,_add);
            return true;
        }
    ...
    {
        address _add = accounts[_id];
        if(chikwama[_add][_id].pin == _pin)
        {
            chikwama[_add][_id].pin = _newPin;
            return true;
        }
        else return false;
    }
